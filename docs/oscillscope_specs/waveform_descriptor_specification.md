...
From the specification, the conversion from raw data to floating point values is:

```
floating_value = VERTICAL_GAIN * data - VERTICAL_OFFSET
```

Where:
- `data` is the raw signed 8-bit integer (when `COMM_TYPE = 0` for byte format)
- `VERTICAL_GAIN` is a float value at byte offset 156
- `VERTICAL_OFFSET` is a float value at byte offset 160

This is the **official formula** from the Siglent oscilloscope specification for converting waveform data to actual measured values.
