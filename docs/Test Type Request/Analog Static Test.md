...
- **EOL Measured Signal**: Displays the EOL measurement signal value
  - Updates in real-time via periodic polling (100ms)
  - Format: `EOL Measured Signal : X.XX <unit>` (unit depends on signal type)
  
- **DUT Feedback Signal**: Displays the DUT feedback signal value
  - Updates in real-time via periodic polling (100ms)
  - Format: `DUT Feedback Signal : X.XX <unit>` (unit depends on signal type)
