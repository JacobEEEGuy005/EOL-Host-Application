...
6. Monitor Test Execution (until test_time_ms elapsed)
   - Action: 
     ...
     5. Display the latest values of `enable_relay_signal`, `enable_pfc_signal`, `pfc_power_good_signal`, and `output_current_signal` (if configured) in the Real-Time Monitoring section so the operator can observe live state changes.
